...
internal class WeaponsDataWatcher : IWeaponsDataWatcher
{
    ...
    private readonly List<IObjectAmmoStashTrigger> _ammoStashTriggers;
    ...
    public void Initialize()
    {
        ...
        _ammoStashTriggers.AddRange(_game.GetObjects<IObjectAmmoStashTrigger>());
        ...
        try
        {
            ...
            foreach (var keyInput in keyInputs)
            {
                ...
                if (keyInput.Key == VirtualKey.ACTIVATE && weaponsData.HasAnyFirearm() &&
                    _ammoStashTriggers.Count > 0 &&
                    _ammoStashTriggers.Any(x =>
                    {
                        var aabb = x.GetAABB();
                        aabb.Grow(2f);
